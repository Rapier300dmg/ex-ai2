...
# Собираем 16 кадров из папки «0»
paths = sorted(glob.glob("data/mouth_frames_train/AVDigits/0/*.jpg"))[:16]

# Выводим первые 4 кадра для визуальной проверки
for p in paths[:4]:
    img = cv2.imread(p)
...
# Загружаем и нормализуем последовательность
seq = [cv2.resize(cv2.imread(p), (64,64)) / 255.0 for p in paths]
seq = np.expand_dims(seq, 0)  # (1, 16, 64, 64, 3)
